...
import SinglePlayerLogic, { Food, Obstacle } from "./SinglePlayerLogic";
...
class EntityGenerator{
    ...
    public generateFood = (): void => {
        ...
        availableBlocksForNewFood = availableBlocksForNewFood.filter(ob => !this.logic.snakeSegments.some(segment => ob.x === segment.x && ob.y === segment.y));
        // Food cannot spawn on other food => Remove the blocks taken by other food
        availableBlocksForNewFood = availableBlocksForNewFood.filter(ob => !this.logic.food.some(food => ob.x === food.x && ob.y === food.y));
        ...
        while (this.logic.food.length < this.logic.getMaxAmountOfFood() && availableBlocksForNewFood.length > 0) {
            const randomIdx: number = Math.floor(Math.random() * availableBlocksForNewFood.length);
            this.logic.food.push({ ...availableBlocksForNewFood[randomIdx] });
